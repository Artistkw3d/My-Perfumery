...
- **طباعة بكمية محددة (Scaled Print)** — زر «طباعة بكمية محددة» بجوار «طباعة PDF» يطلب وزناً إجمالياً (مثلاً 1000g) ويفتح PDF مُكبَّر تلقائياً بنفس النسب (يُضرب G و I والتكلفة بمعامل = `الوزن المطلوب ÷ الوزن الأصلي`). نسب IFRA و L و E3 لا تتأثر لأنها نسب وليست أوزان. كذلك يظهر زر طابعة 🖨️ في رأس كل عمود ضمن نافذة Scale لطباعة PDF بنفس الكمية المعروضة. الترويسة تعرض شارة صفراء «⚙️ مُضاعف لـ Xg» للتمييز.
...
GET  /formula/<id>/print               # صفحة طباعة A4 (يستدعي window.print() تلقائياً)
                                      # ?target_weight=N — يُضرب الأوزان لتطابق الوزن الإجمالي N
