...
## Material photo lookup — multi-vendor (added 2026-08-22)
- **The bug this fixed:** the single-material "بحث عن صورة" button used to search whatever string it got (CAS number, if the material had one, else name) against Fraterworks by slugifying it into a `/products/<slug>.json` guess. Fraterworks product handles are name-based ("geraniol"), not CAS-based ("106-24-1") — so any material with a CAS filled in (i.e. most of them) got a guaranteed-wrong slug guess and always fell through to the manual paste-a-Fraterworks-URL fallback.
- **Fix + expansion:** `_find_material_photo(name, cas)` in `app.py` now tries three vendors in a fixed order, each queried the way it actually supports:
  1. **Fraterworks** — `_fetch_fraterworks_by_name()`, guesses the product handle from the **name** (unchanged mechanism, just now only ever given the name, not the CAS). `robots.txt` disallows `/search`, so this is the only allowed automated path; unmatched materials can still be resolved by the user pasting a Fraterworks product URL by hand (`/api/materials/photo-search/resolve`, unchanged).
  2. **Perfumer's Apprentice** (`shop.perfumersapprentice.com`) — `_fetch_tpa_by_cas()`. Its `/search.aspx?searchterm=<CAS>` genuinely indexes CAS numbers and returns clean matches; the product id parsed from the search-result `<img>` lets us build the full-size image URL directly (`/images/product/large/<id>.jpg`) without a second request.
  3. **Perfumer Supply House** (`perfumersupplyhouse.com`, WooCommerce) — `_fetch_psh_by_cas()`. `/?s=<CAS>&post_type=product` — WordPress redirects straight to the product page on a single match (detected via `resp.geturl()`), or falls back to parsing the first `woocommerce-LoopProduct-link` on the results archive. Note: many PSH product photos are generic stock imagery (iStock IDs), not photos of the actual material — that's a property of their catalog, not a bug here.
  - First hit wins; each source's function returns `None` on no-match/error so the chain moves on silently. `GET /api/materials/photo-search` now takes separate `name`/`cas` query params (frontend passes both) instead of one ambiguous `q`.
- **Bulk update:** "تحديث صور المواد" button on the materials page toolbar → `POST /api/materials/photo-bulk-update`. Runs the same 3-source chain over every material with `photo_file_id IS NULL` (materials that already have a photo are never touched or re-fetched by this — by design, per explicit user requirement) and reports `{updated, not_found, skipped_no_identifier}`. Runs as a background job (see "Bulk background jobs" below), not synchronously.
- Both the single-fetch and bulk paths funnel the actual download+validate+save through one helper, `_download_and_save_material_photo(conn, mid, image_url, allowed_hosts)` — https-only, host-allowlisted per source (`_FRATERWORKS_HOSTS`/`_TPA_HOSTS`/`_PSH_HOSTS`, combined as `_PHOTO_VENDOR_HOSTS` for the manual `fetch_url` action which can point at any of them), 8 MB cap, `Content-Type` must be `image/*`.
...
## Material data bulk enrichment (added 2026-08-22)
