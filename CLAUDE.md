...
- **Draft update-in-place:** "حفظ مسودة" (`action=save` on `POST /api/formula/<fid>/drafts`) always creates a brand-new numbered draft. Added `action=update` (same endpoint, needs `draft_id`) that instead replaces an existing non-final draft's `draft_ingredients` with the formula's current `formula_ingredients`, keeping the same draft id/name — for re-saving edits into the draft you're actively iterating on. Rejects final/approved drafts (`is_final=1`) with a 400-style `success:false`. UI: a small refresh icon (`bi-arrow-repeat`, `.dt-update`) inside each non-final draft tab, `stopPropagation()`'d so it doesn't also trigger `loadDraft()` (clicking the tab body still loads/replaces current ingredients from that draft, unchanged).
