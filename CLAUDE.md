...
- **Bulk update:** "تحديث صور المواد" button on the materials page toolbar → `POST /api/materials/photo-bulk-update`. Runs the same 3-source chain over every material with `photo_file_id IS NULL` (materials that already have a photo are never touched or re-fetched by this — by design, per explicit user requirement) and reports `{updated, not_found, skipped_no_identifier}` in a results modal. Synchronous single request (no background job/polling) — fine for realistic catalog sizes (~1s/material when a source hits, up to ~3×8s timeout when none do); custom/fictional blend names with no CAS (e.g. an in-house "Mix A") predictably land in `not_found` and are meant to be resolved manually per-material afterward.
